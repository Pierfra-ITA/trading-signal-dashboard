...
# CONFIG
st.set_page_config(page_title="Trading Signal App v6.1", layout="wide")
st.title("📈 Trading Signal App – Version 6.1 (Fixed RSI & Full 8 Indicators)")
...
def fetch_data(symbol, days=60):
    ...
    df['Close'] = df['Close'].astype(float)  # ensure 1D Series
    return df
...
def calculate_indicators(df):
    ...
    dmi = ta.trend.DMIIndicator(high=high, low=low, close=close, window=14)
    df['DMI'] = dmi.adx()
    return df
