...
st.title("📈 Trading Signal App – Version 7.3 (Scalar Comparison Fix)")
...
def calculate_adx(df, window=14):
    ...
    plus_di = 100 * (pd.Series(plus_dm).rolling(window=window).mean().values / atr)
    minus_di = 100 * (pd.Series(minus_dm).rolling(window=window).mean().values / atr)

    dx = 100 * np.abs(plus_di - minus_di) / (plus_di + minus_di)
    adx = pd.Series(dx).rolling(window=window).mean().values

    pad_length = df.shape[0] - len(adx)
    adx_full = np.concatenate([np.full(pad_length, np.nan), adx])
    return pd.Series(adx_full, index=df.index, name="DMI").fillna(0)
...
def generate_signals(df):
    ...
    for i in range(1, len(df)):
        ...
        for ind in indicators:
            if pd.isna(today[ind]).any() or pd.isna(yesterday[ind]).any():
                signal_status.append("Neutral")
            elif today[ind].item() > yesterday[ind].item():
