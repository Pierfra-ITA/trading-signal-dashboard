...
# CONFIG
st.set_page_config(page_title="Trading Signal App v6", layout="wide")
st.title("📈 Trading Signal App – Version 6 (8 Real Indicators)")
...
def calculate_indicators(df):
    ...
    df['Volatility'] = np.log(df['Close'] / df['Close'].shift(1)).rolling(10).std() * np.sqrt(252) * 100
    df['ProjOsc'] = projection_oscillator(df['Close'], period=14)
    dmi = ta.trend.DMIIndicator(df['High'], df['Low'], df['Close'], window=14)
    df['DMI'] = dmi.adx()
    return df
